...
@section('title', 'أنواع المؤسسات')
@section('page_title', 'أنواع المؤسسات')
@section('page_subtitle', 'إدارة أنواع المؤسسات (جامعة / معهد)')
        ...
        <tr>
          ...
          <td>
            <div class="flex gap-1">
              ...
              <button class="btn btn-danger py-1 px-2 text-xs" data-delete="{{ route('admin.institution-types.destroy', $type) }}" data-name="{{ $type->name }}">🗑️</button>
            </div>
          </td>
        </tr>
        @empty
...
<div class="modal-overlay" id="createModal">
  <div class="modal-content p-6">
    <div class="flex items-center justify-between mb-5">
      <h3 class="text-lg font-extrabold text-slate-900">➕ إضافة نوع مؤسسة</h3>
      <button class="text-slate-400 hover:text-slate-600 text-xl" data-modal-close>&times;</button>
    </div>
    <form data-ajax="true" action="{{ route('admin.institution-types.store') }}" method="POST">
      @csrf
      <div>
        <label class="label">الاسم <span class="text-rose-500">*</span></label>
        <input class="input" name="name" required placeholder="مثال: جامعة، معهد">
      </div>
...
<div class="modal-overlay" id="editModal{{ $type->id }}">
  <div class="modal-content p-6">
    ...
    <form data-ajax="true" action="{{ route('admin.institution-types.update', $type) }}" method="POST">
      @csrf
      @method('PUT')
      <div>
        <label class="label">الاسم <span class="text-rose-500">*</span></label>
