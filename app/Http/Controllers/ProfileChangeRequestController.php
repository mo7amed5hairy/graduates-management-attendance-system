...
class ProfileChangeRequestController extends Controller
{
    ...
    public function submit(Request $request): JsonResponse|RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'attachments.max' => 'يجب أن لا تزيد المرفقات عن ملف واحد فقط',
            'attachments.*.file' => 'المرفق يجب أن يكون ملفاً صالحاً',
            'attachments.*.mimes' => 'صيغة الملف غير مسموحة. الصيغ المسموحة: JPG, PNG, PDF, DOC, DOCX',
            'attachments.*.max' => 'حجم الملف يتجاوز الحد الأقصى (10 ميجابايت)',
            ...
            'image.image' => 'الصورة الشخصية يجب أن تكون صورة صالحة',
            'image.mimes' => 'صورة الشخصية يجب أن تكون من نوع: jpeg, png, jpg, gif',
            'image.max' => 'حجم الصورة الشخصية يتجاوز الحد الأقصى (5 ميجابايت)',
            'first_name.required' => 'الإسم مطلوب',
            'father_name.required' => 'اسم الأب مطلوب',
            'grandfather_name.required' => 'اسم الجد مطلوب',
            'family_name.required' => 'اللقب مطلوب',
            'email.email' => 'صيغة البريد الإلكتروني غير صحيحة',
            'email.unique' => 'هذا البريد الإلكتروني مستخدم بالفعل',
            'national_id.unique' => 'رقم البطاقة الوطنية مستخدم بالفعل',
            'password.min' => 'كلمة المرور يجب أن تكون 8 أحرف على الأقل',
            'password.confirmed' => 'تأكيد كلمة المرور غير متطابق',
            'date_of_birth.integer' => 'سنة الميلاد يجب أن تكون رقماً صحيحاً',
            'date_of_birth.min' => 'سنة الميلاد غير صالحة',
            'date_of_birth.max' => 'سنة الميلاد غير صالحة',
            'graduation_year.integer' => 'سنة التخرج يجب أن تكون رقماً صحيحاً',
            'graduation_year.min' => 'سنة التخرج غير صالحة',
            'graduation_year.max' => 'سنة التخرج غير صالحة',
            'social_status.in' => 'الحالة الاجتماعية غير صالحة',
